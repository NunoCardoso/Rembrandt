...
Query query = QueryFactory.create("SELECT ?s ?p ?o  WHERE {?s <http://www.w3.org/2000/01/rdf-schema#label> \"${args[0]}\"@pt . ?s ?p ?o}")

QueryExecution qe = QueryExecutionFactory.sparqlService("http://xldb.di.fc.ul.pt/sparql/sparql",query)
