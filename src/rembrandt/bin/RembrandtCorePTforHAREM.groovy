 ...
 */
class RembrandtCorePTforHAREM extends RembrandtCore {
    
     //Map m = Collections.synchronizedMap(new LinkedHashMap(...));
      
    static Logger log = Logger.getLogger("RembrandtMain")
    static final String lang = "pt"
    ...
    SecondHAREMClassificationLabelsPT classes
	...
	number = new NumberRulesPT()
	time = new TimeGroundingRulesPT()
	value = new ValueRulesPT()
	nerules = new NERulesPT()    
	ierules = new InternalEvidenceRulesPT()

	ee_loc = new LocalRulesPT()
	ee_org = new OrganizationRulesPT()
	ee_per = new PersonRulesPT()
	ee_abs = new AbstractionRulesPT()
	ee_thi = new ThingRulesPT()
	ee_eve = new EventRulesPT()
	ee_mas = new MasterpieceRulesPT()
	
	ee2 = new SecondExternalEvidenceRulesPT()
	
	split = new SplitNERulesPT(ierules, saskia)
	classes = new SecondHAREMClassificationLabelsPT()
    }
	...
	public Document releaseRembrandtOnDocument(Document document) {
		...
		document.forcedNEs.each{ne -> 
		   ne = saskia.answerMe(ne) // always get the returned result!
		   // there are cases that I want to compare after/before states. Don't rely on object modification.
	    ...
	    cloned_titleNEs.findAll{ne -> 
          !ne.matchesClassification([SC.time], CC.ExistsAtLeastOneOfThese, CC.AllOfThem, CC.Category) && // leave TIME NEs alone!
	      ...
	      if (ne.hasUnknownClassification()) {
	    	...
	    	// Or preposition in the end, due to bad EM agglomeration and sentence division.
	    	 ...
	    	 if (NEs[i].hasUnknownClassification() && NEs[i].terms.size() > 1)  {
    	    	    if (StopwordsPT.beginningStopwordList.contains(NEs[i]?.terms[0]?.text)) {
	    		NEs[i].termIndex++
	    		NEs[i].terms.remove(0)
	    	    } else if (StopwordsPT.beginningStopwordList.contains(NEs[i]?.terms[-1]?.text)) {
	...
	public boolean suspectedAsProperName(NamedEntity ne) {
	    ...
	    ne.terms.each{term -> 
	      	if (PersonGazetteerPT.firstName.contains(term.text) ||
	      		PersonGazetteerPT.lastName.contains(term.text) || 
	      		PersonGazetteerEN.firstName.contains(term.text) ||
