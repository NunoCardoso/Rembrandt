...
CREATE TABLE IF NOT EXISTS `doc` (
  ...
  `doc_original_id` VARCHAR(255) BINARY NOT NULL,
  `doc_collection` int(10) unsigned NOT NULL,
  `doc_webstore` VARCHAR(255) NULL,
  `doc_version` int(11) DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS `patch` (
  ...
  `pat_version` int(11) NOT NULL,
  `pat_date`  DATETIME DEFAULT NULL, 
  `pat_patch` TEXT NULL)
 ENGINE = InnoDB;

ALTER TABLE `patch` ADD CONSTRAINT FOREIGN KEY (`pat_doc`) 
 REFERENCES `doc` (`doc_id`) ON DELETE CASCADE ON UPDATE CASCADE ;

CREATE INDEX `idx_patch` ON `patch` (`pat_version`, `pat_doc`) ;
...
CREATE TABLE IF NOT EXISTS `commit` (
  `cmm_id` INT UNSIGNED PRIMARY KEY NOT NULL AUTO_INCREMENT ,
  `cmm_doc` INT UNSIGNED NOT NULL,
  `cmm_user` INT UNSIGNED NOT NULL,
  `cmm_date` DATETIME DEFAULT NULL, 
  `cmm_commit` TEXT NULL)
 ENGINE = InnoDB;

ALTER TABLE `commit` ADD CONSTRAINT FOREIGN KEY (`cmm_doc`) 
 REFERENCES `doc` (`doc_id`) ON DELETE CASCADE ON UPDATE CASCADE;
ALTER TABLE `commit` ADD CONSTRAINT FOREIGN KEY (`cmm_user`) 
