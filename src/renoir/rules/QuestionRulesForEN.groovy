...
class QuestionRulesForEN extends QuestionRules {
	...
	static Clause subject1en = new Clause(name:"subject1en",
	    cardinality:Cardinality.One, criteria:Criteria.MeaningMatch,
		pattern:SubjectDB.makeConceptList("en"), options:["case_insensitive":true])
...
/******* #1: RULES FOR QUESTION TYPE *********/
	...
	new Rule(id:'howmanymuch1', description:'How m(any|uch)',
		clauses:[CC.beginSentence1, how1, manymuch1], 
	...
	// 1.2: in which, on which, which
	 new Rule(id:'which1', description:'([io]n?) Which', 
		 clauses:[CC.beginSentence1, ion01, which1], 
		 action:[{MatcherObject o, Question q -> 
...
/******* #2: SUBJECT RULE *********/
	...
	static List rulesToCaptureSubjects = [
	     new Rule(id:'rulesToCaptureSubjects 2.1', description:'Portuguese musicians', 
	      clauses:[placeAdjective1en, subject1en], 
		  action:[{MatcherObject o, Question q -> 
		...
		// How do I distinguish them? Test the "in" or "near" prepositions before it 
			 List<Term> terms = o.getMatchedTermsByClauses([placeAdjective1en, subject1en])
 	...
 	// 2.2
 	   new Rule(id:'rulesToCaptureSubjects 2.2', description:'Musicians [of|in] Portugal',
 		clauses:[subject1en, ClausesEN.inof1nc, placeName1en], 
 		action:[{MatcherObject o, Question q -> 
		
			 List<Term> terms = o.getMatchedTermsByClauses([subject1en, ClausesEN.inof1nc, placeName1en])
...
/****** 3.1 GET GEOSCOPES *****/	
		...
		clauses:[ ClausesEN.into1c,  ClausesEN.the01c, NEGazetteer.NE_LOCAL_1c, ClausesEN.and01c, 
		ClausesEN.in01c, ClausesEN.the01c, NEGazetteer.NE_LOCAL_01c], 
		action:[{MatcherObject o, Question q -> 
		...
		clauses:[ ClausesEN.into1c, ClausesEN.the01c, placeAdjective1en, subject1en], 
		action:[{MatcherObject o, Question q -> 
			List<Term> terms = o.getMatchedTerms()
		...
		clauses:[ ClausesEN.into1c, ClausesEN.the01c, subject1en, 
		ClausesEN.inof1nc, ClausesEN.the01nc, placeName1en], 
		action:[{MatcherObject o, Question q -> 
			List<Term> terms = o.getMatchedTerms()
			log.debug "${o.rule.id} '${o.rule.description}' matched "+terms	
		...
		clauses:[ ClausesEN.neararoundalongbetween1c, ClausesEN.the01c, NEGazetteer.NE_LOCAL_1c, 
		ClausesEN.and01c,  ClausesEN.the01nc, NEGazetteer.NE_LOCAL_01c ], 
		action:[{MatcherObject o, Question q -> 
			List<Term> terms = o.getMatchedTerms()
			log.debug "${o.rule.id} '${o.rule.description}' matched terms "+terms
			List<NamedEntity> nes = o.getMatchedNEs()
		...
		clauses:[ ClausesEN.neararoundalongbetween1c, ClausesEN.the01c, placeAdjective1en, subject1en], 
		action:[{MatcherObject o, Question q -> 
			List<Term> terms = o.getMatchedTerms()
			log.debug "${o.rule.id} '${o.rule.description}' matched "+terms	
		...
		clauses:[ ClausesEN.neararoundalongbetween1c, ClausesEN.the01c, subject1en, 
		    ClausesEN.inof1nc, placeName1en], 
			action:[{MatcherObject o, Question q -> 
				List<Term> terms = o.getMatchedTerms()
				log.debug "${o.rule.id} '${o.rule.description}' matched "+terms		
		...
		clauses:[ ClausesEN.into1c,  ClausesEN.the01c, 
		 LocalGazetteerEN.compasses1nc, ClausesEN.part01c,
		 ClausesEN.of01c, NEGazetteer.NE_LOCAL_1c ], 
		action:[{MatcherObject o, Question q -> 
			List<Term> terms = o.getMatchedTerms()
			log.debug "${o.rule.id} '${o.rule.description}' matched "+terms		
 			List<NamedEntity> nes = o.getMatchedNEs()
		...
		new Rule(id:'rulesToCaptureConditions 3.1.8', 
			...
			clauses:[ ClausesEN.within1c, NEGazetteer.NE_VALOR_QUANTIDADE_1c, ClausesEN.of01c, NEGazetteer.NE_LOCAL_1c ], 
			action:[{MatcherObject o, Question q -> 
				List<Term> terms = o.getMatchedTerms()
				log.debug "${o.rule.id} '${o.rule.description}' matched "+terms		
 			List<NamedEntity> nes = o.getMatchedNEs()
			...
			if (nes.size() == 2) {
				...
				c.predicate = new Predicate(
					dbpediaOntologyProperty:[new DBpediaProperty("location")], //3.4
