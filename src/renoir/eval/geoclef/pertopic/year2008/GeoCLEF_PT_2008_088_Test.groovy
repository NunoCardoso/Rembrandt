 ...
 *
 */
 class GeoCLEF_PT_2008_088_Test extends GroovyTestCase {
        
    Configuration conf 
    GeoCLEF_PerTopic_Test pertopic
    static Logger log = Logger.getLogger("JUnitTest")

    public GeoCLEF_PT_2008_088_Test() {
    ...
    void testTopic() {
		String lang = "pt"
		String topic = "label:088 comunidades de emigrantes portuguesas no mundo"// Sul" 
/*
...
*/
		...
		assert question.sentence*.text == ['comunidades','de','emigrantes','portuguesas','no','mundo']
...
//		assert question.nes[0].terms*.text == ['OCDE']
...
//		assert question.nes[1].classification.find{it == SC.place_human_division}
	...
	// check subjects 
	// games is grounded
		assert question.subject.subjectTerms*.text == ['emigrantes']
		assert question.subject.geoscopeTerms*.text == ['portuguesas']
...
*/	
	/*	assert question.conditions[0].object instanceof QueryGeoscope
		assert question.conditions[0].operator.op == Operator.Locator.In
		assert question.conditions[0].object.ne.terms*.text == ['Oceano','Pacífico']
	*/
		assert question.expectedAnswerTypes[0].DBpediaOntologyResources == ['Category:Portuguese_emigrants']
...
label:088 contents:comunidades contents:"emigrantes" contents:portuguesas contents:mundo
"""
