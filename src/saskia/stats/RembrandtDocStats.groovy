...
class RembrandtDocStats {
	...
	int getTotalNumberOfRembrantedOKDocs() {
		...
		db.getDB().eachRow("SELECT COUNT(*) FROM "+RembrandtedDoc.tablename+
				...
				"doc_proc IN "+DocStatus.whereConditionGoodToProcess(), [collection.col_id], {row -> total = row[0]})
		return total
	}
	...
	int getTotalNumberOfSyncedDocs() {
		...
		db.getDB().eachRow("SELECT COUNT(*) FROM "+RembrandtedDoc.tablename+
				" WHERE doc_collection=? AND "+
				"doc_sync IN "+DocStatus.whereConditionSynced(), [collection.col_id], {row -> total = row[0] })
		return total
	}
