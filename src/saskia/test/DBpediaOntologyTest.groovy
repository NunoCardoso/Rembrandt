 ...
 * @author Nuno Cardoso
 */
class TestDBpediaOntology extends GroovyTestCase {
	...
	void testSuperClassesOf() {
		...
		if (dbpediaontology.getVersion() == "3.2") {
			...
			assert ['Place','PopulatedPlace','Resource'] == dbpediaontology.getSuperClassesOf(shortName1).sort(), \
			"Got "+dbpediaontology.getSuperClassesOf(shortName1).sort() 
			assert ['Place','PopulatedPlace','Resource'] == dbpediaontology.getSuperClassesOf(fullName1).sort()
		}
		if (dbpediaontology.getVersion() == "3.5.1") {
			...
			assert ['Place','PopulatedPlace','Settlement','Thing'] == dbpediaontology.getSuperClassesOf(shortName1).sort(), \
			"Got "+dbpediaontology.getSuperClassesOf(shortName1).sort() 
			assert ['Place','PopulatedPlace','Settlement','Thing'] == dbpediaontology.getSuperClassesOf(fullName1).sort()
		}
	}
	...
	void testBroaderAndNarrowerClass() {
		assert shortName4 == dbpediaontology.getBroaderClassFrom([fullName4,fullName1,shortName3,shortName2]), \
			"Got "+dbpediaontology.getBroaderClassFrom([fullName4,fullName1,shortName3,shortName2])
		assert shortName1 == dbpediaontology.getNarrowerClassFrom([fullName3,fullName2,shortName4,shortName1])
		assert 'Book' == dbpediaontology.getNarrowerClassFrom(['http://dbpedia.org/ontology/Work', 
			'http://dbpedia.org/ontology/Book', 'http://dbpedia.org/ontology/Resource'])
	}
