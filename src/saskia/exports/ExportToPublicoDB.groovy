...
class ExportToPublicoDB extends Export {
	...
	public checkTotal() {
		...
		this.total = collection.getNumberOfDocsWithComment(this.target_table)
	...
	public exporter() {
		...
		for (int i=this.total; i > 0; i -= doc_pool) {
			...
			docs = db.getDBTable("DocTable").getBatchOfDocsWithComment(
	...
	static void main(args) {
		...
		ExportToPublicoDB exporter = new ExportToPublicoDB()
		...
		log.info "******************************************"
		log.info "* This class exports docs into PublicoDB *"
		log.info "******************************************"
