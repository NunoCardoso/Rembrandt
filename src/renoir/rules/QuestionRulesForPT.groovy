...
class QuestionRulesForPT extends QuestionRules {
	...
	static Clause subject1pt = new Clause(name:"subject1pt",
	    cardinality:Cardinality.One, criteria:Criteria.MeaningMatch,
		pattern:SubjectDB.makeConceptList("pt"), options:["case_insensitive":true])
...
/******* #1: RULES FOR QUESTION TYPE *********/
	...
	new Rule(id:'como1', description:'Como', 
	 ...
	 new Rule(id:'como2', description:'Como se chama', 
		clauses:[CC.beginSentence1, como1, ClausesPT.se1c, chama1], 
		action:[{MatcherObject o, Question q -> 
		   q.questionType = QuestionType.Which
		   q.questionTypeTerms = o.getMatchedTermsByClauses([como1, ClausesPT.se1c, chama1])
	...
	new Rule(id:'que1', description:'Em? que', 
		clauses:[CC.beginSentence1, ClausesPT.Eem01c, que1], 
		action:[{MatcherObject o, Question q -> 
		   q.questionType = QuestionType.Which
	...
	new Rule(id:'que2', description:'O que é', 
		clauses:[CC.beginSentence1, ClausesPT.Oo1c, que1, ClausesPT.ee1nc], 
		action:[{MatcherObject o, Question q -> 
		   q.questionType = QuestionType.What
		   q.questionTypeTerms = o.getMatchedTermsByClauses([ClausesPT.Oo1c, que1, ClausesPT.ee1nc])
...
/******* #2: SUBJECT RULE *********/
	...
	static List rulesToCaptureSubjects = [
 		...
 		clauses:[subject1pt, placeAdjective1pt], 
 		action:[{MatcherObject o, Question q -> 
		...
		// assume questions in active mode. I'll get the second one later, in conditions.
		  	 List<Term> terms = o.getMatchedTermsByClauses([subject1pt, placeAdjective1pt])
 	   ...
 	   new Rule(id:'rulesToCaptureSubjects 2.2', description:'Músicos de Portugal',
 	      clauses:[subject1pt, ClausesPT.dnaeosem1c, placeName1pt], 
 		action:[{MatcherObject o, Question q -> 
...
/****** 3.1 GET GEOSCOPES *****/	
		...
		clauses:[ ClausesPT.emnaos1c,  NEGazetteer.NE_LOCAL_1c, ClausesPT.e01c, 
		ClausesPT.emnaos01c, NEGazetteer.NE_LOCAL_01c], 
		action:[{MatcherObject o, Question q -> 
		...
		clauses:[ ClausesPT.emnaos1c, subject1pt, placeAdjective1pt], 
		action:[{MatcherObject o, Question q -> 
			List<Term> terms = o.getMatchedTerms()
		...
		clauses:[ ClausesPT.emnaos1c, subject1pt, ClausesPT.daeos1nc, placeName1pt], 
		action:[{MatcherObject o, Question q -> 
			List<Term> terms = o.getMatchedTerms()
			log.debug "${o.rule.id} '${o.rule.description}' matched "+terms	
		...
		clauses:[ ClausesPT.pertoarredores1c, ClausesPT.daeos1c, NEGazetteer.NE_LOCAL_1c, ClausesPT.e01c, 
			ClausesPT.daeos01c, NEGazetteer.NE_LOCAL_01c], 
		...
		clauses:[ ClausesPT.entre1c, ClausesPT.aos_1c, NEGazetteer.NE_LOCAL_1c, ClausesPT.e01c, 
			ClausesPT.aos_01c, NEGazetteer.NE_LOCAL_01c], 
		...
		clauses:[ ClausesPT.pertoarredores1c, ClausesPT.daeos1c, subject1pt, placeAdjective1pt], 
		action:[{MatcherObject o, Question q -> 
			List<Term> terms = o.getMatchedTerms()
			log.debug "${o.rule.id} '${o.rule.description}' matched terms "+terms
		...
		clauses:[ ClausesPT.pertoarredores1c, ClausesPT.daeos1c, subject1pt, ClausesPT.daeos1nc, placeName1pt], 
		action:[{MatcherObject o, Question q -> 
			List<Term> terms = o.getMatchedTerms()
			log.debug "${o.rule.id} '${o.rule.description}' matched "+terms		
		...
		new Rule(id:'rulesToCaptureConditions 3.1.7', 
			...
			clauses:[ ClausesPT.nao1c, LocalGazetteerPT.compasses1nc, 
			 ClausesPT.daeos1c, NEGazetteer.NE_LOCAL_1c ], 
			action:[{MatcherObject o, Question q -> 
		...
		new Rule(id:'rulesToCaptureConditions 3.1.8', 
			...
			clauses:[NEGazetteer.NE_VALOR_QUANTIDADE_1c, 
			ClausesPT.de01c, NEGazetteer.NE_LOCAL_1c ], 
			action:[{MatcherObject o, Question q -> 
				List<Term> terms = o.getMatchedTerms()
				log.debug "${o.rule.id} '${o.rule.description}' matched "+terms		
 			List<NamedEntity> nes = o.getMatchedNEs()
			...
			if (nes.size() == 2) {
				...
				c.predicate = new Predicate(
					dbpediaOntologyProperty:[new DBpediaProperty("location")]
				)	
...
/***** 3.2 GET PREDICATES FOR GEOSCOPES (or NOT) *****/
		...
		clauses:[ new Clause(name:"birthPlace", cardinality:Cardinality.One, 
			criteria:Criteria.MeaningMatch, collectable:true, 
			pattern:[ 
		   		[answer:new DBpediaProperty("birthPlace"),
				needle:[ [~/nasc(?:idos?|eu|eram)/,~/(?:em|n[ao]s?)/], ['data','de','nascimento']]
			] ] ),
		...
		action:[{MatcherObject o, Question q -> 
			...
			Predicate predicate = new Predicate()
		  	...
		  	predicate.dbpediaOntologyProperty = [predicateMatch.answer] // List
		...
		clauses:[ new Clause(name:"homeTown", cardinality:Cardinality.One, 
		criteria:Criteria.MeaningMatch, collectable:true, 
		pattern:[ 
		      [answer:[new DBpediaProperty("Person/homeTown"), //3.4
		               new DBpediaProperty("hometown")], // 3.5
			needle:[ [~/vivem?/,~/(?:em|n[ao]s?)/] ]
		] ] ),
			NEGazetteer.NE_LOCAL_1c ], 
		action:[{MatcherObject o, Question q -> 
