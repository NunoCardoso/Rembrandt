...
class ExportToDir extends Export {
	...
	public exporter() {
		...
		int rows = 0 
		int total = collection.getNumberOfDocuments()
      ...
      for (int i=total; i > 0; i -= docs) {
         ...
         List docs = db.getDBTable("DocTable")
				.getBatchOfDocsOrderedByOriginalDocId(collection, docs, processed)
         ...
         if (!docs) {
 		   ...
 		   docs.each {doc ->
		    	out.write("<META DOC_ORIGINAL_ID=\""+doc.doc_original_id+"\">\n")
				out.write(doc.doc_content.trim()+"\n")
				status.exported++	
		   }
	...
	static void main(args) {
		...
		ExportToDir exporter = new ExportToDir()
