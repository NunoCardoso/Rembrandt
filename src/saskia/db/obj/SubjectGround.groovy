...
class SubjectGround extends DBObject implements JSONable {
	...
	public updateValue(column, value) {
		...
		def res = getDBTable().getSaskiaDB().getDB().executeUpdate(
			"UPDATE ${getTable().getTablename()} SET ${column}=? WHERE sgr_id=?",
			[newvalue, sgr_id])
		return res
	}
	...
	public Long addThisToDB() {
		def res = getDBTable().getSaskiaDB().getDB().executeInsert(
			"INSERT INTO ${getTable().getTablename()}(sgr_subject, sgr_geoscope, sgr_dbpedia_resource, "+
	...
	public int removeThisFromDB() {
		...
		def res = getDBTable().getSaskiaDB()..getDB().executeUpdate(
			"DELETE FROM ${getTable().getTablename()} WHERE sgr_id=?", [sgr_id])
