 ...
 */
class Pagico_noStem_QueryGeneration extends Pagico_QueryGeneration {
    ...
    public Pagico_noStem_QueryGeneration(topic_file, query_file, collection, lang, Boolean stem) {
	super(topic_file, query_file, lang)
	renoir = new Renoir(Configuration.newInstance(), collection, lang, stem)
	
    }
 
    void generate(bm25_k1 = 1.2d, bm25_b = 0.75d) {
	...
	topics.each{topicline -> 
	   ...
	   topicline = topicline.replaceAll(/Pagico_(\d+):/, "label:\$1")
	   topicline = topicline.replaceAll(/#(.*)$/, "")
	   ...
	   log.debug "Initial query string w/configuration: ${query.toString()}" 
	   super.logf.append "Initial query string w/configuration: ${query.toString()}\n" 
	...
	String homedir = Configuration.newInstance().get("rembrandt.home.dir",".")
	String fileseparator = System.getProperty("file.separator")
	def db  = saskia.db.database.SaskiaMainDB.newInstance()
	List l = saskia.db.table.CollectionTable.getFromName(db, "Pagico")
	def o = new Pagico_noStem_QueryGeneration(
		"Pagico_original_topics.txt", 
		"pagico_nostem.query", 
		homedir+fileseparator+"index"+fileseparator+"db-main"+fileseparator+"col-"+l[0].col_id, 
		"pt", 
		false)
	o.generate()
	}
