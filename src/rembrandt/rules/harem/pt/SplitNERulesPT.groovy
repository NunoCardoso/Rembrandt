...
class SplitNERulesPT extends SplitNEDetector {
	...
	//	println "Rule 1.1 in action"
		NEs.generateALT(o.original_ne, [o.saskia_nes[1]], "RULE: SplitNERulesPT 1.1 ACTION: Generate ALT, include NE ${o.saskia_nes[1]}")		
		 }
	...
	//	println "Rule 1.2 in action"
	        NEs.removeNEs(o.original_ne)
	        NEs.addNEs(o.saskia_nes, ConflictPolicy.JustAdd, "RULE: SplitNERulesPT 1.2 ACTION: added from splitted NE ${o.original_ne}")	 
	 }
	...
	//	println "Rule 1.3 in action"
	         NEs.generateALT(o.original_ne, [o.saskia_nes[1]],  "RULE: SplitNERulesPT 1.3 ACTION: Generate ALT, include NE ${o.saskia_nes[1]}")	    
	 }    
	...
	//	println "Rule 1.4 in action"
	        NEs.removeNEs(o.original_ne)
	        NEs.addNEs(o.saskia_nes, ConflictPolicy.JustAdd, "RULE: SplitNERulesPT 1.4 ACTION: added from splitted NE ${o.original_ne}")		 
	}   	
	 ...
	 action:[{SplitNEMatcherObject o, ListOfNE NEs  -> o.lang=lang}, 
	    ...
	    */
		...
		o.saskia_nes[1]?.matchesClassification([SC.place],  [AllOfThese, AllOfThem,  rembrandt.obj.ClassificationCriteria.Category]) ) {
			// there's a ListOfNE called NEs where I can add the split. 
		NEs.generateALT(o.original_ne, [o.saskia_nes[1]], "RULE: SplitNERulesPT 2A.1 ACTION: Generate ALT, include NE ${o.saskia_nes[1]}")		
	   }
		...
		o.saskia_nes[1]?.matchesClassification([SC.place], [AllOfThese, AllOfThem,  rembrandt.obj.ClassificationCriteria.Category]) ) {
		// there's a ListOfNE called NEs where I can add the split. 
		NEs.generateALT(o.original_ne, [o.saskia_nes[1]], "RULE: SplitNERulesPT 2A.2 ACTION: Generate ALT, include NE ${o.saskia_nes[1]}")			
	    }
	    return true
	   } ] ) ,
	 ...
	 action:[{SplitNEMatcherObject o, ListOfNE NEs  -> o.lang=lang}, 
	    ...
	    */
		...
		o.saskia_nes[1]?.matchesClassification([SC.place], [AllOfThese, AllOfThem,  rembrandt.obj.ClassificationCriteria.Category]) ) {
			// there's a ListOfNE called NEs where I can add the split. 
		NEs.generateALT(o.original_ne, [o.saskia_nes[1]], "RULE: SplitNERulesPT 2B.1 ACTION: Generate ALT, include NE ${o.saskia_nes[1]}")			
	   }
		...
		o.saskia_nes[1]?.matchesClassification([SC.place], [AllOfThese, AllOfThem,  rembrandt.obj.ClassificationCriteria.Category]) ) {
				// there's a ListOfNE called NEs where I can add the split. 
		NEs.generateALT(o.original_ne, [o.saskia_nes[1]], "RULE: SplitNERulesPT 2B.2 ACTION: Generate ALT, include NE ${o.saskia_nes[1]}")		
	    }
	    return true
	   } ] ) , 
	 ...
	 action:[{SplitNEMatcherObject o, ListOfNE NEs  -> o.lang=lang}, 
	    ...
	    */
		...
		o.saskia_nes[1]?.matchesClassification([SC.organization], 
			[ExistsAtLeastOneOfThese, AllOfThem,  rembrandt.obj.ClassificationCriteria.Category]) ) {
			// there's a ListOfNE called NEs where I can add the split. 
		NEs.generateALT(o.original_ne, [o.saskia_nes[1]], "RULE: SplitNERulesPT 3B.1 ACTION: Generate ALT, include NE ${o.saskia_nes[1]}")				   
	   }	
	    return true
	} ] )  
      ]
