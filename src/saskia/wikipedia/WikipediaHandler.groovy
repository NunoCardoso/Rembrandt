 ...
 * &lt;/mediawiki&gt;
 */
class WikipediaHandler  extends DefaultHandler {
    	...
    	// open inlinks file
   	  	...
   	  	f.eachLine { line ->  	  	
   	  		...
   	  		if (matcher) {
   	  	       ...
   	  	       db.execute WikipediaDB.insertInlinks, [matcher.group(1), matcher.group(2), matcher.group(3)]
   	  		}
   	  		pb.incrementValue()
   	  		pb.printProgressBar()			  		
   	  	}	  
   ...
   private addToDocument(
   	...
   	try {
	      ...
	      def inlinks = db.rows("SELECT source, anchor FROM "+conf.get('db.table.inlinks')+" WHERE target=${numid}")	      
	      def outlinks2 = db.rows("SELECT target, anchor FROM "+conf.get('db.table.inlinks')+" WHERE source=${numid}")	      
