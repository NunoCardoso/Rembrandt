...
public class AdminTaskMapping extends WebServiceRestletMapping {
    ...
    public AdminTaskMapping(SaskiaDB db) {
        ...
        JSONanswer = {req, par, bind ->
            ...
            ServerMessage sm = new ServerMessage("AdminTaskMapping", lang, bind, session, processlog)  
            ...
            if (action == "list") {
					Map h 
               try {
                  h = Task.listTasks(limit, offset, column, value)
               } catch(Exception e) {
            ...
            if (action == "update") {
               ...
               try {
                ...
                return sm.statusMessageWithPubKey(res, tsk.toMap(), user.usr_pub_key)	
            }            
		...
		//			 if (tsks) return sm.statusMessage(-1, i18n.servermessage['task_already_exists'][lang])
                ...
                return sm.statusMessageWithPubKey(0, tsk.toMap(), user.usr_pub_key)	
            }
