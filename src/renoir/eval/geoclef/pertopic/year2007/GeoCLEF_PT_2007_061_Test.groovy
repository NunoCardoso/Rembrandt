 ...
 *
 */
 class GeoCLEF_PT_2007_061_Test extends GroovyTestCase {
        
    Configuration conf 
    GeoCLEF_PerTopic_Test pertopic
    static Logger log = Logger.getLogger("JUnitTest")

    public GeoCLEF_PT_2007_061_Test() {
    ...
    void testTopic() {
		String lang = "pt"
		String topic = "label:061 Acidentes de avião perto de cidades da Rússia"
...
*/
		...
		assert question.sentence*.text == ['Acidentes','de','avião','perto','de','cidades','da','Rússia']
		assert question.questionType == QuestionType.None
		assert question.questionTypeTerms.size() == 0

	// check detected NEs 
		assert question.nes.size() == 1
		assert question.nes[0].terms*.text == ['Rússia']
