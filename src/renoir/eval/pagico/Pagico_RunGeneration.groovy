...
class Pagico_RunGeneration {
    ...
    public Pagico_RunGeneration(String query_file, String qrel_file, String run_file, 
 ...
 // initialize by reading the topics and qrels
	...
	queryfile = homedir+fileseparator+"resources"+fileseparator+"eval"+
        ...
        qrelfile = homedir+fileseparator+"resources"+fileseparator+"eval"+
            fileseparator+"pagico"+fileseparator+"qrels"+fileseparator+qrel_file
        runfile = homedir+fileseparator+"resources"+fileseparator+"eval"+
            fileseparator+"pagico"+fileseparator+"runs"+fileseparator+run_file
        logfile = homedir+fileseparator+"resources"+fileseparator+"eval"+
            fileseparator+"pagico"+fileseparator+"logs"+fileseparator+run_file+".log"

	   def db = collection.getDBTable().getSaskiaDB()
       indexdir = homedir+fileseparator+"index"+fileseparator+"db-"+db.LABEL+fileseparator+"col-"+collection.col_id
    ...
    void submit(RenoirQuery rq) {
            ...
            for (int i = from; i < to; i++) {
                ...
                runf.append "$label " + result["doc_original_id"]+"\n"
            }
            logf.append "===\n"
