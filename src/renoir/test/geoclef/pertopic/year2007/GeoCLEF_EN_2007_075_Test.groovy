 ...
 *
 */
 class GeoCLEF_EN_2007_075_Test extends GroovyTestCase {
        
    Configuration conf 
    GeoCLEF_PerTopic_Test pertopic
    static Logger log = Logger.getLogger("JUnitTest")

    public GeoCLEF_EN_2007_075_Test() {
    ...
    void testTopic() {
		String lang = "en"
		String topic = "label:075 Violation of human rights in Burma"
		...
		assert question.sentence*.text == ['Violation','of','human','rights','in','Burma']
...
//		assert question.nes[0].terms*.text == ['St.','Paul','\'s','Cathedral']
	//	assert question.nes[0].classification.find{it == SC.place_human_construction}
		assert question.nes[0].terms*.text == ['Burma']
	...
	// check conditions 
		...
		assert question.conditions[0].object.ne.terms*.text == ['Burma']
