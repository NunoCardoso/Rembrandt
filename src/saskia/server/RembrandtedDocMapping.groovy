 ...
 *
 */
public class RembrandtedDocMapping extends WebServiceRestletMapping {
    ...
    public RembrandtedDocMapping() {
        ...
        JSONanswer = {req, par, bind ->
            ...
            ServerMessage sm = new ServerMessage("RembrandtedDocMapping", lang, bind, session, processlog)  
            ...
            /*********************/
                ...
                if (!Collection.canAdmin(user, collection)) 
					 ...
					 if (user.usr_max_docs_per_collection < number_rdocs) 
					    return sm.statusMessage(-1, i18n.servermessage['max_number_documents_per_collection_reached'][lang])
 				  	  ...
 				  	  try {
                    RembrandtedDoc rdoc = new RembrandtedDoc(
                ...
                return sm.statusMessage(0, rdoc.toMap())	
            }
            ...
            /** 1.6 createNE **/
            /****************/
           // nes will be added to pool only, check if doc_sync allows that.
