 ...
 *
 */
 class GeoCLEF_PT_2006_048_Test extends GroovyTestCase {
        
    Configuration conf 
    GeoCLEF_PerTopic_Test pertopic
    static Logger log = Logger.getLogger("JUnitTest")

    public GeoCLEF_PT_2006_048_Test() {
    ...
    void testTopic() {
		String lang = "pt"
		String topic = "label:048 Pescas na Terra Nova e na Gronelândia" 
...
*/
		...
		assert question.sentence*.text == ['Pescas','na','Terra','Nova','e','na','Gronelândia']
...
/*		assert question.nes.size() == 2
		assert question.nes[0].terms*.text == ['Terra','Nova']
		assert question.nes[0].classification.find{it == SC.place_human_division}
		assert question.nes[1].terms*.text == ['Gronelândia']
	...
	// check conditions 
		...
		assert question.conditions[0].object.ne.terms*.text == ['Terra','Nova']
		assert question.conditions[1].object instanceof QueryGeoscope
		assert question.conditions[1].operator.op == Operator.Locator.In
		assert question.conditions[1].object.ne.terms*.text == ['Gronelândia']
