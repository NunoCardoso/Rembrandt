...
import saskia.converters.CHAVE2DocumentConverter
...
class ImportChave {
	...
	public ImportChave() {
	    ...
	    c2rd = new CHAVE2DocumentConverter()
	...
	public HashMap importDoc(File file, collection_) {
	    ...
	    // c2rd converts from CHAVE format to Doc format
	    c2rd.clear()
	    String text = c2rd.parse(file)
	   
	    // let's check if there's a doc on the DB
	    Doc doc = Doc.getFromOriginalDocIDandCollectionAndLang(		
		...
		if (doc) {
		    ...
		    if ((!ynae) || (ynae == "y") || (ynae == "n")) {
				...
				while (!(ynae == "a" || ynae == "e" || ynae == "n" || ynae == "y")) {
				    println "Overwriting Doc ${id} on DB? ([y]es, [a]lways, [n]o, n[e]ver)"
		    ...
		    if ((ynae == "y") || (ynae == "a")) {
				Doc newdoc = new Doc(
					...
					doc_original_id:id,
					doc_lang:lang, 
					doc_date_created:date_created.time, 
					doc_content:text
				)
				newdoc.replaceThisToDB()				
				newdoc.changeProcStatusInDBto(DocStatus.READY) // mark it so next time we'll not use it
				log.debug "Doc ${doc.doc_original_id} in DB overwrited."

		    } else if ((ynae == "n") || (ynae == "e")) {
				log.debug "Found ${doc.doc_original_id} in Saskia DB, skipping."
		    }
		} 
	   // I need:
	   else {
     		doc = new Doc(doc_original_id:id, doc_collection:collection, doc_lang:lang,
	...
	static void main(args) {
	    ...
	    String usage = "Usage: saskia.imports.ImportChave [dir] [lang] [target-collection]\n"+
		  "Learn it.";
		...
		ImportChave c2r = new ImportChave()
