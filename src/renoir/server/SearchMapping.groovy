	...
	public SearchMapping() {
		...
		//	if (!api_key) return sm.noAPIKeyMessage()
         ...
         User user
			...
			if (!Collection.canRead(user, collection) && !user.isSuperUser())
		...
		// PREPARE EXPLANATION
			...
			// now let's convert doc_original_id from the collection to the text
		   	try {
					RembrandtedDoc rdoc = RembrandtedDoc.getFromOriginalDocIDandCollection(r["doc_original_id"], collection)
