...
public class SourceDoc extends DBObject implements JSONable {
	...
	public int changeProcStatusInDBto(DocStatus status) {
		def res = getDBTable().getSaskiaDB().getDB().executeUpdate(
				"UPDATE ${getDBTable().getTablename()} SET sdoc_proc=? WHERE sdoc_id=? and "+
	...
	public int addDocID(Long doc_id) {
		...
		def res = getDBTable().getSaskiaDB().getDB().executeUpdate(
				"UPDATE ${getDBTable().getTablename()} SET sdoc_doc=? WHERE sdoc_id=? and "+
	...
	public Long addThisToDB() {
		try {
			if (sdoc_content) {
				String key = getDBTable().webstore.store(sdoc_content, SaskiaWebstore.VOLUME_SDOC)
				log.info "Got content (${sdoc_content.size()} bytes), wrote to Webstore SDOC, got key $key"
				def res = getDBTable().getSaskiaDB().getDB().executeInsert("INSERT INTO ${SourceDoc.tablename}"+
	...
	public Long replaceThisToDB() {
		try {
			if (sdoc_content) {
				...
				if (sdoc_webstore) {
					try {
						getDBTable().webstore.delete(sdoc_webstore, SaskiaWebstore.VOLUME_SDOC)
