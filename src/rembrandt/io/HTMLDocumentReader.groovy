 ...
 */
class HTMLDocumentReader extends Reader {
    
    int numberanchor = 0
    int numbertag = 0
    
    // A (Anchor) is already handled... no need to include here.
    List<String> allowedHTMLtags = ["B","I","P","BR","HR","H1","H2","H3","H4","H5","H6","OL","UL","LI"]
    List<String> newlinetags = ["BR","P","H1","H2","H3","H4","H5","H6", "OL","UL","LI"]
	...
	htmltext = htmltext.replaceAll(/(?si)<!-- DOC ID="(.*)" LANG="(.*)" -->/) {
	    all, g1, g2 -> docid=g1; lang=g2; return ""}
	
        // Protect the anchors, they might have pre-grounded info
        htmltext = htmltext.replaceAll(/(?si)(?:<\s*A\s*HREF=["']?)(.*?)(?:["']?\s*>)([^<]*)(?:<\s*\/A\s*>)/) {all, g1, g2 ->
            // if anchor starts with http://, that's not pre-grounded info. We are not interested
            if (g1.startsWith("http://")) return g2
            else return "${saveanchor(g1)}${g2} ${endanchormark} "
        }
