 ...
 *
 */
 class GeoCLEF_PT_2007_066_Test extends GroovyTestCase {
        
    Configuration conf 
    GeoCLEF_PerTopic_Test pertopic
    static Logger log = Logger.getLogger("JUnitTest")

    public GeoCLEF_PT_2007_066_Test() {
    ...
    void testTopic() {
		String lang = "pt"
		String topic = "label:066 Economia no Bósforo"
/*
...
*/
		...
		assert question.sentence*.text == ['Economia','no','Bósforo']
		assert question.questionType == QuestionType.None
		assert question.questionTypeTerms.size() == 0

	// check detected NEs 
		assert question.nes.size() == 2
		assert question.nes[0].terms*.text == ['Economia']
		assert question.nes[0].classification.find{it == SC.abstraction_discipline}
		assert question.nes[1].terms*.text == ['Bósforo']
		assert question.nes[1].classification.find{it == SC.place_physical}
...
//		assert question.conditions[0].object.geoscopeTerms*.text == ['Rússia']
		...
		assert question.conditions[0].object.ne.terms*.text ==  ['Bósforo']
...
label:066 contents:"Economia" contents:"Bósforo" entity:Economics entity:Bosphorus
