...
class ImportPublico extends Import {
	...
	public importer() {
		...
		for (int i=this.total; i > 0; i -= this.doc_pool) {
			...
			// adding the table as a comment
				Doc d = addDoc(doc.docid, content, doc.lang, doc.date_created, ""+this.table)
	...
	static void main(args) {
		...
		ImportPublico importer = new ImportPublico()
