...
CREATE INDEX `idx_doc_proc` ON `RembrandtPool`.`doc` (`doc_proc`) ;
CREATE INDEX `idx_doc_sync` ON `RembrandtPool`.`doc` (`doc_sync`) ;
CREATE INDEX `idx_doc_edit` ON `RembrandtPool`.`doc` (`doc_edit`) ;
...
CREATE INDEX `idx_sdoc_original_id` ON `RembrandtPool`.`source_doc` (`sdoc_original_id`) ;
CREATE INDEX `idx_sdoc_doc` ON `RembrandtPool`.`source_doc` (`sdoc_doc`) ;
...
CREATE TABLE IF NOT EXISTS `RembrandtPool`.`user_on_collection` (
  ...
  `uoc_own` TINYINT(1) DEFAULT FALSE, 
...
ALTER TABLE `RembrandtPool`.`ne` ADD KEY `RembrandtPool`.`ne_lang`( `ne_lang`);
...
-- -----------------------------------------------------
-- Table `RembrandtPool`.`subject`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `RembrandtPool`.`subject` ;

CREATE TABLE IF NOT EXISTS `RembrandtPool`.`subject` (
  `sbj_id` INT UNSIGNED PRIMARY KEY AUTO_INCREMENT,
  `sbj_subject` TEXT NOT NULL)
ENGINE = InnoDB;

-- -----------------------------------------------------
-- Table `RembrandtPool`.`subject_ground`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `RembrandtPool`.`subject_ground` ;

CREATE TABLE IF NOT EXISTS `RembrandtPool`.`subject_ground` (
...
ALTER TABLE `RembrandtPool`.`subject_ground` ADD CONSTRAINT FOREIGN KEY (`sgr_subject`) 
 REFERENCES `RembrandtPool`.`subject` (`sbj_id`) ON DELETE CASCADE ON UPDATE CASCADE ;
ALTER TABLE `RembrandtPool`.`subject_ground` ADD CONSTRAINT FOREIGN KEY (`sgr_geoscope`) 
 REFERENCES `RembrandtPool`.`geoscope` (`geo_id`) ON DELETE CASCADE ON UPDATE CASCADE ;
...
INSERT INTO `RembrandtPool`.`geoscope`(geo_name, geo_woeid) VALUES("",0);
