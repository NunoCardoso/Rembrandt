	...
	return $(this).each(function(){
		...
		var createChart = {
			...
			line: function(area){
				...
				//write X labels
				// Nuno Cardoso
				debug(canvas.width()+" "+canvas.css('width'))
				var canvaswidth=parseInt(canvas.css('width').replace(/px/,""))
				var canvasheight=parseInt(canvas.css('height').replace(/px/,""))
				// now, replace canvas.width() and canvas.height()
				var xInterval = canvaswidth / (xLabels.length -1);
				...
				//write Y labels
				var yScale = canvasheight / topValue;
				var liBottom = canvasheight / (yLabels.length-1);
