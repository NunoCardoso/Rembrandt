 ...
 *
 */
class QuestionAnalyserTest extends GroovyTestCase{

	Configuration conf

	public QuestionAnalyserTest() {
	...
	void testString1() {
		...
		assert question.subject?.categoryWikipediaAsDBPediaResource == [
			"Category:Portuguese_musicians"
		]
		// grounded classifications
		assert question.subject?.categoryHAREM == [CC.person_individual]
		assert question.subject?.ontologyDBpediaClass == [
		...
		assert question.expectedAnswerTypes[0].resolvesTo == \
		ExpectedAnswerType.Type.DBpediaOntologyResource

		/** check answers **/
		assert question.answer.sort() == [
			'http://dbpedia.org/resource/David_Fonseca'
		]
